...
public class DisplayInvoiceSummaryController implements Initializable {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(DisplayInvoiceSummaryController.class);
...
//	private Label netProfit;
	...
	public void searchInvoices() {
		try {
			...
			LOGGER.info("Searching Invoices for selected date  : {}  to {}", sDate, eDate);
			eDate = new Timestamp(eDate.getTime()+ 86400000);
	...
	private double calculateNetProfit(InvoiceDTO invoiceDTO) {
		...
		if (invoiceDTO.getItemInvoices() != null && !invoiceDTO.getItemInvoices().isEmpty()) {
			for (ItemInvoiceDTO itemInvoiceDTO : invoiceDTO.getItemInvoices()) {
				double amountDifference = itemInvoiceDTO.getProductSellingPrice() - itemInvoiceDTO.getItem().getCostPrice();
